...
# 
# <p>
#     In this project you will be building an email spam filter. It is due <strong>October 25th</strong> (late due <strong>October 27th</strong>)</p>
# 
...
# 
# <p><strong>Getting Help:</strong> You are not alone!  If you find yourself stuck  on something, contact the course staff for help.  Office hours, section, and the <a href="https://edstem.org/us/courses/25800/discussion/">EdStem</a> are there for your support; please use them.  If you can't make our office hours, let us know and we will schedule more.  We want these projects to be rewarding and instructional, not frustrating and demoralizing.  But, we don't know when or how to help unless you ask.
# 
# 
# 
...
# Gradients: 
# <ol>
#     <li> Ridge Regression: $=\frac{-2}{n}\sum_{i=1}^n x_i^T(y-x_iw)+2\lambda w$ </li>
#     <li> Logistic Regression: $=\sum_{i=1}^n \frac{-y_ix_i}{1+exp\{y_i(w^Tx_i)\}}$ </li>
#     <li> Hinge loss: $=\sum_{i=1}^n \max \left(-y_i(x_i),0\right)+2\lambda w$
#    </li>
#    </ol>
...
def ridge(w,xTr,yTr,lmbda):
    ...
    gradient = np.zeros(d)
    for i in range(n):
        loss = loss + (np.dot(np.transpose(w),xTr[i])-yTr[i])**2 
        gradient = gradient + ((yTr[i]-(np.dot(w,xTr[i])))*xTr[i])         
    loss = loss / n + np.dot(np.transpose(w),w)*lmbda
    gradient = gradient*(-2) / n + 2*lmbda*w
...
def adagrad(func,w,alpha,maxiter,eps,delta=1e-02):
    ...
    zd = np.zeros(w.shape[0])
    
    ## fill in your code here
    w = w
    i = 0
    w_next = np.zeros(w.shape[0])
    while (i<1000 and np.linalg.norm(w_next - w) < delta):
        a = func(w)
        g = a[1]
        l = a[0]
        zd = zd + g**2
        w_next = w - (alpha* g/(np.sqrt(zd+eps)))
        losses[i] = l 
        w = w_next
        i = i+1
...
def linclassify(w,xTr):
    ## fill in your code here
    mat = np.matmul(w.transpose(),xTr.transpose()) 
...
def logistic(w,xTr,yTr):
    ...
    gradient = np.zeros(d)
    for i in range(n):
        loss = loss + np.log(1+np.e**(-yTr[i]*np.dot(w,xTr[i])))
        gradient = gradient + (-yTr[i]*xTr[i])/(1+np.e**(yTr[i]*np.dot(w,xTr[i])))        
    return loss , gradient
#</GRADED>
...
def hinge(w,xTr,yTr,lmbda):
    ...
    gradient = np.zeros(d)
    for i in range(n):
        loss = loss + np.where(yTr[i]*(np.dot(w,xTr[i]))<1,1-yTr[i]*(np.dot(w,xTr[i])),0)
        gradient = gradient + np.where(yTr[i]*np.dot(w,xTr[i])<1,-yTr[i]*xTr[i],0)         
    loss = loss + np.dot(np.transpose(w),w)*lmbda
    gradient = gradient + 2*lmbda*w
...
# <h3>Competition <b>(Extra Credit, up to 10 points)</b></h3>
# 
...
def trainspamfiltercomp(xTr, yTr):
    ...
    lmbda = .5
    eps = 1e-06
    #w = adagrad(lambda weight: logistic(weight, xTr, yTr), np.zeros(xTr.shape[1]), 1, 1000, 1e-05)[0]
    # w = adagrad(lambda weight: hinge(weight, xTr, yTr, lmbda), np.zeros(xTr.shape[1]), 1, 1000, 1e-05)[0]
    w = adagrad(lambda weight: ridge(weight, xTr, yTr, lmbda), np.zeros(xTr.shape[1]), 1, 1200, eps)[0]
    return w
#</GRADED>
